...
class WhatsApp(QRunnable):
    ...
    def timer_count(self):
        ...
        while count_down < self.timer:
            ...
            self.signals.timer_progress_completed.emit()
...
class UserContactApp(QMainWindow):
    ...
    def refresh_progress(self, total_workers):
        # Function to refresh the progress bars
        total_progress = sum(worker.signals.progress.value() for worker in self.worker_progress.values())
        self.progress_bar.setValue(total_progress)
        if total_workers > 0:
            overall_progress = total_progress / total_workers
        else:
            overall_progress = 0
        self.progress_bar.setValue(overall_progress)

    @pyqtSlot()
    def worker_completed(self, uuid):
    ...
    def abort_function(self):
        for worker in self.worker_progress.values():
            worker.abort()  # Call abort method to stop the worker
        self.connect_button.setEnabled(True)
